...
export default function MathGame() {
  ...
  const [_resetClicks, setResetClicks] = useState(0);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: saveState and updateUI are stable helpers triggered by state changes
  useEffect(() => {
    saveState();
  ...
  function drawProblem(p: Problem) {
    ...
    const canvasBg = (
      rootStyles.getPropertyValue("--colorNeutralBackground2") || "#1b1b1b"
    ).trim();
    const boxBg = (
      rootStyles.getPropertyValue("--colorNeutralBackground4") || "#2f2f2f"
    ).trim();
    const textPrimary = (
      rootStyles.getPropertyValue("--colorNeutralForeground1") || "#ffffff"
    ).trim();
    const textSecondary = (
      rootStyles.getPropertyValue("--colorNeutralForeground3") || "#a1a1a1"
    ).trim();
    const strokeColor = (
      rootStyles.getPropertyValue("--colorNeutralStroke1") || "#333333"
    ).trim();
    const accent = (
      rootStyles.getPropertyValue("--colorBrandBackground") || "#0078d4"
    ).trim();
    ...
    const boxW = displayWidth <= 375 ? 60 : displayWidth <= 768 ? 70 : 100;
    const midBoxW = displayWidth <= 375 ? 90 : displayWidth <= 768 ? 110 : 140;
    const arrowLen = displayWidth <= 375 ? 20 : displayWidth <= 768 ? 30 : 50;
    const boxH = 60;
  ...
  function checkAnswer() {
    ...
    if (Number.isNaN(userAnswer) || !currProblem) {
      setFeedback("請輸入數字");
      return;
    }
    ...
    <div>
      ...
      <div className="status-bar">
        ...
        <div className="status-item">
          分數: <span className="score-text">{score}</span>
        </div>
        <div className="status-item">
          正確: <span id="correctVal">{correctCount}</span>
        </div>
      </div>
      ...
      <div className="progress-container">
        <div
          ...
          aria-label="進度"
          style={{
            width: `${Math.max(0, Math.min(100, (score / TARGET_SCORE) * 100))}%`,
          }}
          aria-valuenow={score}
          aria-valuemin={0}
      ...
      <div className="canvas-container">
        ...
        <div className="input-group">
          <label className="question-label" htmlFor="userAnswer">
            答案:
          </label>
          <input
            type="number"
            id="userAnswer"
            ref={inputRef}
            className="fui-Input-field input-small"
            aria-label="答案"
            onKeyDown={handleInputKey}
          />
          <button
            type="button"
            className="fui-Button fui-Button--primary btn-submit"
            onClick={checkAnswer}
          >
            提交
          </button>
          <button
            type="button"
            id="showAnswerBtn"
            className="fui-Button fui-Button--secondary btn-show-answer ml-8"
            onClick={showAnswer}
          >
            顯示答案
          </button>
        </div>
        <div className="feedback-area" id="feedbackArea" aria-live="polite">
          {feedback}
        </div>
      </div>
      ...
      <div className="button-container">
        <button
          type="button"
          className="fui-Button fui-Button--primary btn-report"
          onClick={showReport}
        >
          📊 成績單
        </button>
        <button
          type="button"
          className="fui-Button fui-Button--warning btn-reset"
          id="btnReset"
          onClick={handleReset}
        >
          ⚠️ 重置所有進度
      ...
      {showReportModal && (
        <div className="modal-overlay" id="reportModal">
          <div className="modal-content">
            <button
              type="button"
              className="modal-close fui-Button fui-Button--light"
              onClick={closeReport}
              aria-label="關閉"
            >
              ×
            ...
            <div className="stats-grid">
              <div className="stat-box">
                <small>最終分數</small>
                <br />
                <strong className="value value--accent">{score}</strong>
              </div>
              <div className="stat-box">
                <small>總題數</small>
                <br />
                <strong className="value">{correctCount + wrongCount}</strong>
              </div>
              <div className="stat-box">
                <small>正確題數</small>
                <br />
                <strong className="value value--green">{correctCount}</strong>
              </div>
              <div className="stat-box">
                <small>正確率</small>
                <br />
